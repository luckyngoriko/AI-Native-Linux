...
| Consumes       | S3.1 (RecordType vocabulary + RetentionClass + segment model + hash chain algorithm), S6.2 (EvidenceGrade enum), S6.4 (INV-005, INV-014, INV-015, INV-016), S5.1 (Subject canonical id format), S4.1 (`(scope, group_id, user_id)` triple), S0.1 (BLAKE3 + JCS hash convention) |
