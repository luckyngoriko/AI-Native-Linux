...
| Consumes       | **Imports vocabulary from** L0 INV catalog (INV-002 AI-proposes-never-executes, INV-008 default-deny, INV-013 AI-cannot-perform-system-admin, INV-014 no-proof-no-completion, INV-017 sandbox-floor-constitutional); S5.2 Vault Broker (`KEY_SIGN` / `KEY_VERIFY` operation kinds); S3.2 Sandbox Composition (`SandboxProfile` closed shape); S9.1 Recovery Boundary (`RecoveryMutableScope`, `RecoveryMode = RECOVERY` enums); S8.1 Network Policy (`NetworkOutboundManifest` closed shape); S8.5 Firmware Trust (firmware-blob trust shape, `FirmwareUpdateClass` enum, `FirmwareTrustResult` enum — reciprocates the W11-A bilateral inversion); S3.1 Evidence Log (`RecordType` vocabulary, `FOREVER` retention class); S4.1 Namespace Layout (`installable_scope` closed enum). **Requires for correctness** S5.3 Approval Mechanics (runtime `request_approval` outcome and `EXACT_ACTION` binding consumed at install time); S1.1 Capability Catalog (runtime declared-capability resolution at first-run audit). |
...
Direct attempts by an AI subject to bypass the request flow (e.g. emitting `package.install` directly) are hard-denied at S2.3 with `AIInstallInitiationBlocked`; FOREVER `APP_AI_DIRECT_INSTALL_ATTEMPTED_BLOCKED` evidence (per the AIInstallInitiationBlocked hard-deny added in S2.3 Wave 9; cite S0.4 §4.2 enforcement map); the AI subject cannot retry within a back-off window.
