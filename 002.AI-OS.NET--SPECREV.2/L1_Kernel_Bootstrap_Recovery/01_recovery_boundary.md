...
| Consumes       | L0 INV-001, INV-004, INV-012, INV-022; S5.1 Identity Model (`SessionClass.RECOVERY`, `_system` scope subjects, recovery session 8-hour cap); S2.3 Policy Kernel hard-deny `RecoveryRequiredForSystemMutation`; S5.4 Emergency Override (`STRONG_SOLO` recovery-only); S4.1 Namespace Layout (`/aios/system/...` recovery-only mutation classes); L7.1 Surface + Composition Model (recovery surface stack restriction)                             |
...
2. **Mutation does not imply "free hand".** Even in recovery, mutations to these paths emit `RECOVERY_OPERATION_PERFORMED` evidence (FOREVER retention) with the scope and target path. The audit trail is identical for a benign policy bundle update and a malicious one — the difference shows up in `INVARIANT_LOOSENING_REJECTED` (when the malicious bundle is rejected) or in the post-recovery operational record.
...
- [ ] `RecoveryMutableScope` is a closed enum with exactly eight values; each maps to a `/aios/system/...` path.
