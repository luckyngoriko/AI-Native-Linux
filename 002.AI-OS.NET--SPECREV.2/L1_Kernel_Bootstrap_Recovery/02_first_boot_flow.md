...
| Consumes       | L0 INV-001 (recovery independent of L5), INV-004 (recovery boundary preserved), INV-005 (evidence append-only), INV-006 (web UI localhost-only by default), INV-008 (default-deny in policy), INV-012 (recovery required for system mutation), INV-013 (AI cannot perform system admin), INV-014 (no proof, no completion), INV-018 (vault never leaks raw secrets), INV-022 (recovery aesthetically distinct); S9.1 Recovery Boundary (`RecoveryStage` FSM, `RecoveryEntryReason`, `RecoveryMutableScope`, the three constitutional roots `/`, `/root`, `/aios`); S9.3 Dedicated Kernel Pipeline (A/B kernel slot semantics — first-boot uses the generic kernel exclusively); S2.3 Policy Kernel (initial bundle compile + load); S5.1 Identity Model (`SubjectKind`, `_system` scope subjects, `SessionClass`); S5.2 Vault Broker (master key generation, TPM seal, hardware-key seal); S4.1 Namespace Layout (`/aios/system/...` paths first-boot writes); S3.1 Evidence Log (append-only; `RecordType` extension queue); L0 §3 governance bundle (initial invariant bundle signature) |
