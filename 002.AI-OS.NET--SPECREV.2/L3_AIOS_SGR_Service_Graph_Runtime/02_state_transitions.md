...
| S2.4  | consumer   | references the verification primitive vocabulary for health probes. Truthful posture (Wave 16 correction): `transition_succeeded` / `transition_failed` are **not** yet in the S2.4 closed primitive enum. Health probes for per-transition outcomes currently compose existing S2.4 primitives (`evidence_exists` on the §9 transition record + `policy.decision` on the trigger). A dedicated probe pair is queued for a future S2.4 Wave when per-transition observability justifies a dedicated shape. |
