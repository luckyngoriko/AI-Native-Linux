...
| Consumes       | **Imports vocabulary from**: S0.1 Action Envelope + Lifecycle (cross-cutting, type-level), S2.4 Verification Engine (verification primitive vocabulary, type-level), S3.2 Sandbox Composition (`SandboxProfile` shape, type-level), S3.1 Evidence Log RecordType + retention vocabulary (type-level), S2.3 Policy Kernel (decision shape + hard-deny vocabulary, type-level), S5.3 Approval Mechanics (approval-request shape, type-level), S5.4 Emergency Override (`NonOverridableClass` shape, type-level), L4.2 Vault Broker (capability-binding shape, type-level), L0 invariant catalog (closed-id reference). **Integration points (NOT runtime-required)**: L3 issues `EvaluatePolicyForAction` calls into S2.3 and emits records absorbed by S3.1; if S2.3/S3.1 are temporarily unavailable, actions queue (S2.3 → `policy_pending`) or evidence buffers in WAL until log resumes — the gRPC surface contract holds. The L4 vault unavailable case → action waits in `executing` queue without secret access; no L3 contract breakage. |
|                |                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `ROLLED_BACK`      | `ROLLED_BACK`           | Terminal. Adapter rollback completed successfully after an execution or verification failure. Entering this state writes evidence via the `ROLLBACK_COMPLETED` RecordType (S3.1 Appendix A ID 11) with the originating action id, the verification failure that triggered rollback, and the adapter-reported `RollbackOutcome`. |
...
message ValidateActionResponse {
  ...
  string action_request_id = 4;          // L3-internal handle, "actrq_" + 26-char base32 ULID (S0.1 §3.2). Distinct from S0.1 `act_` (action envelope id): the `actrq_` id is the runtime's queue handle for one execution attempt; the envelope's `act_` id remains the canonical identity carried in evidence.
}
