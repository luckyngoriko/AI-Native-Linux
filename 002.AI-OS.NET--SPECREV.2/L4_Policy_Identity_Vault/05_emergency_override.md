...
Three constitutional invariants frame this entire spec. INV-001 (Default deny) is the upstream rule that produced the hard-deny in the first place. INV-007 (Hard-deny cannot be silently overridden) is the rule this whole sub-spec exists to honor: every override is loud, signed, evidenced, and counted against a per-subject monthly cap. INV-005 (Evidence is append-only) is the rule that prevents an override or a misuse of an override from being scrubbed from the historical record. The three invariants together mean an override is always visible, always costly, and never silent.
...
- `POLICY_DENY_LIST_DELETE` binds INV-007 (per the override's own purpose: hard-deny cannot be silently overridden) and INV-008 (default deny). Deleting deny-list entries silently degrades the constitution.
...
INV-007 ("Hard-deny cannot be silently overridden") is satisfied here in two senses: (1) overrides are loud, costly, and FOREVER-retained; (2) `NonOverridableClass` overrides are not possible at all.
...
The recovery-mode special case binds INV-001 (recovery independent of L5), INV-004 (recovery boundary), INV-007 (hard-deny cannot be silently overridden — recovery is loud, signed, and FOREVER-evidenced), and INV-022 (recovery aesthetic distinct — the override prompt's chrome inherits the recovery theme so the operator cannot mistake the moment for normal mode).
...
| L0   | consumer            | INV-001, INV-003, INV-004, INV-005, INV-007 (hard-deny cannot be silently overridden), INV-008, INV-018, INV-019, INV-020, INV-022                                                                                                     |
...
7. Tamper-detection scheduled audit notes the attempt; operator alert raised. INV-005 (evidence append-only) and INV-007 (hard-deny cannot be silently overridden) hold.
