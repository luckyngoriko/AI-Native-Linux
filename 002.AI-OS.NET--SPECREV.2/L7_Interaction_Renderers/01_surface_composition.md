...
| Consumes       | **Imports vocabulary from**: S0.1 action targets (type-level), S1.3 object scope binding (type-level), S2.3 policy decisions (type-level), S3.1 evidence record schema (type-level), S3.2 sandbox profile (type-level), S4.1 namespace catalog (type-level), S5.1 identity (type-level), L0 invariants INV-019..INV-022 (closed-id reference), S8.2 L8 GPU resource model (`gpu_capability_class` closed enum + per-group `VkDevice` brokering API — type-level shape co-defined with L8). **Note (architectural)**: S8.2 is a higher-numbered-layer reference at L8; this is currently a vocabulary import, but the GPU-capability vocabulary could be relocated to a cross-cutting GPU-capability contract that both L7 and L8 import — flagged for W12+ sweep. The renderer does not require L8 GPU resource model operational at L7 contract validation time; raw GPU access for renderers is brokered at runtime via S8.2's per-group device handle. |
