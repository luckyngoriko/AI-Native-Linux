...
- **S2.3** to add five closed condition fields: `target.device_class`, `target.device_trust_class`, `target.removable`, `target.driver_provenance`, `target.firmware_trusted`. Constitutional hard-deny `RemovableDeviceForbiddenForAI` is **already covered** by `AISystemAdminBlocked`; no new hard-deny is required. **(promoted in S2.3 Wave 17 §29 — applied 2026-05-23, closing Tier 6 audit GAP-003.)**
