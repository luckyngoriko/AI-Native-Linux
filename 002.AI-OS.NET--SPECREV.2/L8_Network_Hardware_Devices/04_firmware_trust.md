...
| Consumes       | **Imports vocabulary from**: L0 INV closed-id references (INV-002, INV-004, INV-005, INV-008, INV-013, INV-014, INV-018); S0.1 Action Envelope (cross-cutting, type-level); S5.3 Approval Mechanics (`ApprovalStrength`, `EXACT_ACTION` binding — type-level); S5.4 Emergency Override (`NonOverridableClass`, `SCOPE_TOO_BROAD` — type-level); S3.1 Evidence Log (`RecordType` vocabulary, retention classes `STANDARD_24M`/`EXTENDED_60M`/`FOREVER` — type-level); S2.3 Policy Kernel hard-deny vocabulary (type-level); S10.1 Capability Runtime (action lifecycle FSM — type-level); S5.2 Vault Broker (`KEY_VERIFY` capability schema — type-level); S5.1 Identity Model (`_system:service:firmware-update` subject canonical-id, `is_ai = false` — type-level). **Peer (intra-L8)**: S8.3 Hardware Graph (referenced abstractly via `DeviceClass` and `firmware_version`; this spec does not freeze the hardware-graph wire shape). **Inverted direction (vocabulary owned here, consumed upward)**: ~~S11.1 Repository Model (firmware bundles as `PackageKind` extension; `PackageVerificationResult` mirrored)~~ — this is the inversion: L10 must import the firmware-trust vocabulary from S8.5 (this spec is the canonical owner of `FirmwareUpdateClass`/`FirmwareTrustResult`/firmware-blob trust shape); the L10 packaging pipeline imports vocabulary from S8.5, not the other way around. Reframed: **L10/S11.1 imports vocabulary from S8.5**; this upward `Consumes` declaration is struck. **Lower-numbered runtime substrate**: S9.1 Recovery Boundary (`RecoveryStage`, `RecoveryMutableScope`, `RecoveryMode = RECOVERY` for non-overridable scopes — peer L1 dependency for recovery-mode-gated firmware operations), S9.2 First-Boot Flow (firmware blob set consumed at first-boot HARDWARE_FIT prerequisites — peer L1 dependency), S9.3 Dedicated Kernel Pipeline (`firmware_blob_set` is a build input — peer L1; this is S9.3 _consuming_ L8.5's output, declared here for cross-reference clarity, not a runtime requirement on S9.3). |
...
**Closed-enum fail-closed rule for the legacy LVFS carve-out.** The parenthetical "(or RSA-PSS for some legacy LVFS blobs)" in the `SIGNATURE_FAILED` row above is constrained by a closed enum. Anything outside Ed25519 is denied unless the chain hop is annotated `LVFS_LEGACY` AND the parent `MirrorSemantic = ORIGIN`. The closed `LegacySignaturePrimitive` enum (LVFS_RSA_PSS_SHA256 only — see §18 IDL) bounds the carve-out; no other primitive can be silently admitted at any chain hop.
...
- **Signature primitive selection.** The contract names Ed25519 as the default. Some legacy LVFS firmware uses RSA-PSS with SHA-256; the LVFS metadata signature itself is GPG-signed historically. The implementer may accept the legacy primitives at the LVFS path only, gated by a closed `LegacySignaturePrimitive` enum scoped to LVFS metadata; vendor-signed proprietary and AIOS publisher root signatures MUST be Ed25519. The closed enum is:

  ```proto
  enum LegacySignaturePrimitive {
    LEGACY_SIGNATURE_PRIMITIVE_UNSPECIFIED = 0;
    LVFS_RSA_PSS_SHA256                    = 1;
  }
  ```

  Any primitive not enumerated above is denied at the verification step; the carve-out is bounded to a single value (LVFS_RSA_PSS_SHA256) with a single admissible parent annotation (`LVFS_LEGACY` + `MirrorSemantic = ORIGIN`) per §3.4.
